...
class GameOverMenu(Menu):
    ...
    Attributes inherited from superclass:
        ...
        run_display: A boolean value which controls whther a menu should be
        displayed or not.
        event_handler: The EventHandler object attached to the game to handle
        input from the player.
        renderer: The Renderer object attached to the game which renders
        different menus.
    ...
    Attributes:
        ...
        status: The status is passed to the Renderer object to display whether
    ...
    def display_menu(self):
        """Displays the Game Over Menu to the player until the player navigates
        to a different menu.
        """
        ...
        while self.run_display:
            ...
            self._check_input()
            self.renderer.render_game_over_menu(self.replayx, self.replayy,
                                        self.quitx, self.quity, self.status)
            self.renderer.draw_cursor(self.cursor_rect.x, self.cursor_rect.y)
            self.renderer.blit_screen()

    def _move_cursor(self):
        """Displays the cursor to the player as it moves across different
        options on the Game Over Menu.
